...
module "create_vm" {
  ...
  subnet_id             = data.terraform_remote_state.vnet-state.outputs.subnet1_id          # subnet details to deploy vm into
  ...
  docker_install_dir    = "${var.docker_install_file.base_dir}/${var.docker_install_file.file_dir}" # Create the full dir based on onedrive base and path
  docker_install_script = var.docker_install_file.file_name
