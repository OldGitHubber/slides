...
publicIP = {
  ...
  sku         = "Standard"
  domain_name = "poor-joke" # This should be unique within the region. Azure will add the .uksouth.cloudapp.azure.com
}

NIC_name        = "poor-joke" # Network interface card name
ip_config_name  = "main"
...
vm_spec = {
  name       = "poor-joke"
 # size       = "Standard_B2ats_v2" # The free one B1s. May need B2s for database
  size       = "Standard_B2s" # The free one B1s. May need B2s for database
  admin-name = "tony"
}
...
docker_install_script = "docker-az-tf.sh" # Same as docker-az.sh but without adding docker to the group or terraform hangs
...
copy_config = [
  ...
  {
    # Copy the .sql file
    source = "../dbinit/jokes.sql"
    destination = "/home/tony/dbinit/jokes.sql"
  },
]

# Provided again as opposed to compose having to dig it out of the above list
compose_filename = "../compose.yaml" 


push_images          = ["oldgitdocker/poor-joke:latest"] # List of images to push
