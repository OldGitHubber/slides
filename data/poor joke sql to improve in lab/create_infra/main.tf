...
resource "azurerm_linux_virtual_machine" "demo_vm" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "curl -k 'https://www.duckdns.org/update?domains=${var.domain}&token=${var.dns_token}&ip='", # Send IP to duckDNS. Duck will pull it from the request -k avoids tls cert check
    ]
    connection {
      type        = "ssh"
      user        = "tony"
...
#   virtual_machine_id   = azurerm_linux_virtual_machine.demo_vm.id
...
resource "null_resource" "cert_transfer" {
  ...
  connection {
    ...
    private_key = file(var.private_key_file)
    host        = azurerm_linux_virtual_machine.demo_vm.public_ip_address
  }
}

// Create a directory for the database .sql file if using one. Need to set permissions
// Make sure the VM exists
resource "null_resource" "create_dbinit_dir" {
  depends_on = [azurerm_linux_virtual_machine.demo_vm]
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo mkdir -p /home/tony/dbinit",
      "sudo chmod 755 /home/tony/dbinit",
      "sudo chown ${var.vm_spec.admin-name}:${var.vm_spec.admin-name} /home/tony/dbinit" # Default will be root so need to change
    ]
  }
}
...
# Make sure the database dir exists before copying files if on of the files is the .sql initialisation at dir dbinit
resource "null_resource" "copy_config_files" {
  depends_on = [azurerm_linux_virtual_machine.demo_vm, null_resource.create_dbinit_dir]
  triggers = {
...
# Before starting containers, make sure the vm exists, images have been pushed and config files transferred
resource "null_resource" "start_containers" {
  depends_on = [azurerm_linux_virtual_machine.demo_vm, null_resource.build-and-push-images, null_resource.copy_config_files]
