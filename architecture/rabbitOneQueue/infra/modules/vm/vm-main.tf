...
# Add docker to the user group. This is handy if you need to login and run docker manually without having to use sudo all the time
# Get the password from the file and login to docker hub
# Run compose to pull images and start containers
# The env var is just an example of how to pass a variable env var into the compose script to be copied into env vars should you need to 
resource "null_resource" "start_containers" {
 ...
 triggers = {
  ...
  provisioner "file" {
    source      = "${var.compose_source_dir}/${var.compose_filename}"
    destination = "${var.dest_dir}/${var.compose_filename}"
    connection {
      type        = "ssh"
      user        = "tony"
      private_key = file(var.private_key_file)
