...
variable "state_path" {
  type        = string
  description = "Path to the state file of the network"
...
variable "publicIP" {
  type = object({
    ...
    domain_name = string  # e.g. kong. Azure adds the region and cloudapp.azure.com
  })
}
...
# Need this to point to the compose file to build the images
